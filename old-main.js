...
const LEVELS = {
    ...
    4: {
        ...
        maze: generateMaze(50, 35, 0.12, 0.03), // Larger area with significantly more traps
        time: 420 
    },
    5: {
        name: "The Abyssal Core",
        maze: generateMaze(65, 45, 0.15, 0.02), // Maximum size and trap density
        time: 600 
    }
};
const TILE_MAP = {
...
new p5((p) => {
    ...
    let victoryMessage = '';
    let lossReason = '';
    let leaderboardView = 'overall'; // "overall" or 1, 2, 3, 4, 5
    ...
    let playerName = localStorage.getItem('torchbound_username') || '';
    ...
    p.keyPressed = () => {
        if (currentGameState === GAME_STATE.NAME_INPUT) {
            if (p.keyCode === p.ENTER) {
                ...
                // Only add printable characters, max 12
                playerName += p.key;
            }
            return; // Stop here so typing 'P' doesn't pause the game
        }
    ...
    function onLevelComplete() {
        ...
        saveScore(currentLevel, playerName, currentScore);
    ...
    function saveScore(level, name, score) {
        // 1. Retrieve the existing leaderboard for this level
        let leaderboard =
            JSON.parse(localStorage.getItem(`torchbound_lv${level}`)) || [];

        // 2. Check if this player already has an entry
        let playerEntryIndex = leaderboard.findIndex(
            (entry) => entry.name.toLowerCase() === name.toLowerCase(),
        );

        if (playerEntryIndex !== -1) {
            // 3. If they exist, only update if the new score is a "Personal Best"
            if (score > leaderboard[playerEntryIndex].score) {
                leaderboard[playerEntryIndex].score = score;
            }
        } else {
            // 4. If they don't exist, add them as a new entry
            leaderboard.push({ name: name, score: score });
        }

        // 5. Sort by highest score and keep the top 5
    ...
    function drawMenu() {
        ...
        drawButton(
            ...
            () => {
                playerName = ''; // Clear current name
                localStorage.removeItem('torchbound_username'); // Clear the saved name
    ...
    function drawVictory() {
        ...
        // Display Leaderboard
        let topScores = getLeaderboard(currentLevel);
        p.textSize(24);
        p.fill(200);
        p.text(
        ...
        topScores.forEach((entry, i) => {
            let yPos = 180 + i * 35;
            // Highlight current player in Yellow, others in White
    ...
    function drawGlobalLeaderboard() {
        p.textAlign(p.CENTER, p.CENTER);
        p.fill(255);
        p.textSize(42);
        ...
        let labels = ['OVERALL', 'LV 1', 'LV 2', 'LV 3', 'LV 4', 'LV 5'];
        let views = ['overall', 1, 2, 3, 4, 5];

        views.forEach((v, i) => {
            let btnX = p.windowWidth / 2 - 250 + i * 100;
            drawButton(
                labels[i],
                btnX,
                130,
                ...
                90,
                40,
            ); // Updated drawButton to support size
        });
    ...
    function drawButton(label, x, y, onClick, btnW = 300, btnH = 50) {
        // Check hover
        let isHovered =
            p.mouseX > x - btnW / 2 &&
            p.mouseX < x + btnW / 2 &&
